...
The rest of the this analysis focusses on the last six decades, for two reasons: First, most of the increase in annual average temperature occured within that time period, so I think any consequent changes in weather will probably then, too. Second, data from the prior period come from only one station, are partially incomplete, and some variables simply weren't measured before about 1957, e.g. sunshine duration and wind speed.

I'll also utilize LOESS smoother to make subtler short-term trends more apparent. Linear models certainly can tell us something about long-term trends - 60 years, in this case - but a 10-year trend isn't exactly unimportant, especially since part of my motivation stems from my own decade of observations in Berlin. I think searching for a linear relationship between temperature and any other variable might also be a waste of time given the complexity of the systems involved. 

The following plot groups of some weather variables should clarify my intent, as well as providing a bird's eye view of where Berlin's climate may be heading. The strongest trends can be seen in average annual temperature and wind speed, the latter of which seems to be decreasing as the former increases. There might be a connection there - for example, warmer weather might lead to more high-pressure fronts, which promise fair weather - but I really can't conclude much based on the data at hand.

With the following code I create a subset of the Berlin data for 1957-2017, then I plot nine variables of interest.
...
It makes sense that there is an inverse relationship between cloud cover and sunshine duration (less clouds = more sunshine). Relative humidity seems to follow a pattern similar to that of cloud cover. Vapor pressure is obviously closely related to temperature, as even it's fluctuations closely match those of temperature.

After watching The Organic Chemistry Tutor's excellent [tutorial](https://www.youtube.com/watch?v=BqFVtlQa-2w) on the relationship between relative humidity and vapor pressure, I think something like the following might be happening: Vapor pressure is a function of temperature, i.e. it increases as temperature increases (this is a fact). Relative humidity tells us how much moisture is in the air compared to how much moisture *could* be in the air at a given temperature. Formally, relative humidity is the quotient of the partial pressure of water vapor divided by equilibrium vapor pressure, where the partial pressure of water vapor represents the actual amount of water vapor in the air. Here's the equation (Thanks [Wikipedia](https://en.wikipedia.org/wiki/Relative_humidity)!):

$$
\phi ={\frac {p_{\mathrm {H_{2}O}}}{p_{\mathrm {H_{2}O} }^{*}}}
$$

Rising temperatures cause vapor pressure to rise as well, but partial pressure may increase more slowly or even remain fairly stable, depending on the amount of water available in the environment. An increasing $p_{\mathrm {H_{2}O} }^{*}$ and stable $p_{\mathrm {H_{2}O} }$ would drive down the $\phi$, even though the absolute moisture content of the air is actually increasing. The upshot is that more heat and moisture [lead to stronger storms](https://earthobservatory.nasa.gov/Features/ClimateStorms/page2.php).

In the next cluster of  I apply the LOESS smoother I mentioned above to the same data, which offers a more nuance picture.
...
Cloud cover and humdity are, in fact, increasing as of approximately 1987, and temperature and vapor pressure do not align as neatly as they appear to above. Nevertheless, I suspect my interpretation of the data might still hold. First of all, it is expected that that partial pressure, and thus relative humidity, eventually catches up to vapor pressure as increases taper off and more and more water evaporates due to the increased heat. Also, the effects of an increase in temperature could lag behind the cause as a result of more complex changes in the broader syster, e.g. the mitigating effects of flora or large bodies of water.

The one variable that seems to contradict my theory is precipitation, which has remained comparatively stable despite a couple of pronounced spikes in the latter years. However, these are annual means, but the types of storm fronts I am concerned with most likely occur in during warmest part of the year. The next cluster of time series zooms in on annual changes for the months May-August.
...
Predictably, the proportion of days with maximum temperatures above 25°C has gone up since 1957, particularly in July and August, but also in May and September. The summer months show a great deal of variation over the years. Also interesting is the fact that June has remained fairly stable even as other months show dramatic increases.
...
Looking at the next plot, the average number of hours of daily sunshine appear to have remained fairly stable for most months. There is a long-term increase to be seen in April, and to a lesser degree in March. In June and September there is long decline followed by an ascent to previous levels. There are no obvious downward trends. I take this to mean that Berlin's weather has not necessarily become "worse" in general, as more constant cloudy weather and rainfall should affect daily sunshine averages.
...
> **Edit**: I've since realized that sample size *n* can affect *p*, i.e. large sample sizes can lead to small *p*-values if $H_0$ is true. As a result, testing for significance might not be entirely relevant here, as my samples are rather large (*n* = 7,503). But this doesn't really affect the rest of my analysis anyway.
...
There are a couple of potential problems with this interpretation, particulary if we want to . First, the upward trend is really confined to the period 2000-2017, and it doesn't look like much against the backdrop of the previous four decades. In that context it might be little more than a hiccup. Second, the increases in May and August aren't entirely consistent with the decreases we see in daily precipitation. (**Note**: The fact that air pressure is lowest in the summer is [not a surprise](https://www.researchgate.net/post/Why_does_atmospheric_pressure_show_higher_values_in_December_and_loweer_in_July).)

Albeit, I still think there might be a trend here, for the following reasons:
...
2. The overall trends for the individual months show remarkable similarities to the corresponding trends in average daily precipitation, which seems to indicate that these kinds of shifts are a decent proxy for weather patterns.
3. Regarding the inconsistencies in May and August: A drastic drop in air pressure can also be a sign of windstorms, or of storms strong storm fronts that pass over Berlin without leaving much rain behind.
...
Obviously I can't confidently draw too many conclusions based on the data I have. There are, nonetheless, a few things that I am confident about: First, Berlin is getting warmer. On that the data is clear. Second, it is [well known](https://earthobservatory.nasa.gov/Features/ClimateStorms/page2.php) that temperature has an effect on weather systems generally, and storm intensity specifically, although there is a question of whether or not this is happening around Berlin.

The data seem to indicate that temperature has had, and continues to have, an effect on Berlin's weather. One uncontroversial finding is that vapor pressure has increased as a result of rising temperatures, which has in turn caused an increase in humidity. Given the known link between heat, moisture, and storm strength, it shouldn't be too far off the mark to surmise that storms in and around Berlin are increasing in intensity. The data also hint at exactly this development: Precipitation has increased in the warmest months even as the proportion of rainless days and average daily sunshine has remained relatively stable, suggesting that storms have become more intense without necessarily becoming more frequent.
