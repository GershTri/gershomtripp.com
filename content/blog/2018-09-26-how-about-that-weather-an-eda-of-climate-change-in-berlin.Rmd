...
```{r}
berlin_temp <- berlin_data %>%
    filter(Variable == "Avg. Temp. (°C)") %>% 
    group_by(Year, Week) %>%
    summarise(Value = mean(Value)) %>% 
    full_join(berlin_data %>%
                  filter(Variable == "Avg. Temp. (°C)"
                         ) %>%
                  group_by(Week) %>% 
                  summarise(Week_mean = mean(Value, trim = .2)),
              by ="Week"
              ) %>% 
    mutate(Mean_dev = Value - Week_mean)

berlin_temp %>% 
    ggplot(aes(x = Year, y = Week)) +
    geom_raster(aes(fill = Mean_dev))

berlin_temp$pos_neg <- ifelse(berlin_temp$Value < 0, -1, 1)

berlin_temp %>% 
    mutate(Mean_dev_trans = Mean_dev^2 * pos_neg) %>% 
    ggplot(aes(x = Year, y = Week)) +
    geom_raster(aes(fill = Mean_dev_trans))
```
