...
yelp_df %>% 
    ...
    ggtitle("Thirty Restaurants with the Most Yelp Reviews")
...
city_summary <- yelp_df %>% 
    ...
    mutate(bus_tot_city = length(unique(yelp_df$id)),
           preval_city = round(cat_tot_city/nrow(yelp_df), 2)) %>% 
    arrange(desc(preval_city))


yelp_df %>% 
    group_by(district, categories.title) %>% 
    summarise(cat_tot_dist = n()) %>% 
    mutate(bus_tot_dist = length(unique(yelp_df$id)),
           preval_dist = round(cat_tot_city/nrow(yelp_df), 2)) %>% 
    arrange(desc(preval_c))
