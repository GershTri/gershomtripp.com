...
## Excursus: Deciding on a Metric

Ilia Blinderman and Amber Thomas [draw](https://pudding.cool/2018/03/neighborhoods/) on Katie Hempenius' reasoning in her [analysis](https://katiehempenius.com/post/yelp-map-of-san-francisco/) of San Francisco's neighborhoods. With the dual goals of "identify[ing] interesting trends" and "normaliz[ing] for the effects of commercial versus residential usage" in mind, Katie arrives at the following formula for "incidence rate":
...
Fortunately, the biggest issue is also the easiest to address. Simply put, the
above formula does _not_ describe the incidence rate. The [incidence rate](https://www.cdc.gov/ophss/csels/dsepd/ss1978/lesson3/section2.html) is a measure of _new cases_ in a specified period of _time_. Time, however, doesn't play
a role in the Katie's formula. Instead, the formula describes the proportion of all category members that can be found in a given district, multiplied by a constant.

Less of an issue, but perhaps relevant in terms of computational resources, is
the fact that the second half of the equation - the constant - scales all the proporitions upwards without actually changing the relationships between them.
That is to say, it has no effect on the neighborhood-level rankings, which is 
what we are interested in. A simplified formula will produce the same results:
...
1. a neighborhood is home to a category whose members make up less than 1% of all categories, but draws a disproportionate amount of interest, as measured by the number of reviews? Zoos, botanical gardens, etc. come to mind.
2. a neighborhood is home to a large proportion of a category's city-wide members, but they collectively make up a small fraction of all businesses in the district?
3. one category member is responsible for a large amount of interest, as measured by review count? For example, in the dot chart _Thirty Businesses with the Most Yelp Reviews_ above, Mustafa's Gemüse Kebap is the clear winner, but most kebap/kebab stands don't get anywhere near that number of reviews.
...
# calculate category review proportions for city
rev_prop_city_df <- yelp_df %>% 
    group_by(categories.title) %>% 
    summarise(rev_cat_city = sum(review_count)) %>% 
    mutate(rev_sum_city = sum(rev_cat_city),
           rev_prop_city = rev_cat_city/rev_sum_city)

# calculate category prevalence * proportion reviews for city
preval_prop_city_df <- full_join(preval_city_df,
                                 rev_prop_city_df,
                                 by = "categories.title"
                                 ) %>% 
    mutate(preval_x_prop_city = preval_city * rev_prop_city)
...
# calculate category review proportions for district
rev_prop_dist_df <- yelp_df %>% 
    group_by(district, categories.title) %>% 
    summarise(rev_cat_dist = sum(review_count)) %>% 
    full_join(yelp_df %>%
                  group_by(district) %>%
                  summarise(rev_sum_dist = sum(review_count)),
              by = "district"
              ) %>% 
    mutate(rev_prop_dist = rev_cat_dist/rev_sum_dist)

# calculate category prevalence * proportion reviews for district
preval_prop_dist_df <- full_join(preval_dist_df,
                                 rev_prop_dist_df,
                                 by = c("district", "categories.title")
                                 ) %>% 
    mutate(preval_x_prop_dist = preval_dist * rev_prop_dist)

# calculate 
preval_df <- full_join(preval_prop_dist_df,
                       preval_prop_city_df,
                       by = "categories.title"
                       ) %>% 
    mutate(preval_diff = preval_x_prop_dist - preval_x_prop_city) %>% 
