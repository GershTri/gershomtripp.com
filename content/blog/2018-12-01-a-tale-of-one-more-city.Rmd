...
berlin_map <- get_stamenmap(berlin_coord, maptype = "toner-background", darken = .8)
...
```
```{r}
ggplot(yelp_df %>% select(id, coordinates.latitude, coordinates.longitude, district) %>% 
                   distinct()) +
    geom_point(aes(coordinates.longitude, coordinates.latitude, color = district),
               size = 0.1, alpha = .3) + 
    coord_fixed(1.7) +
    ggtitle("Distribution of All Yelp-Reviewed Businesses") + 
    theme(axis.text = element_blank(),
...
test_df <- a_df %>% 
    mutate(r_cmplx = (nd/nc) * (sum(nc)/sum(nd)),
           r_cmplx_noconst = nd/nc,
           r_diff = (nd/sum(nd)) - (nc/sum(nc))
           )

test_df %>% arrange(r_cmplx)
test_df %>% arrange(r_diff)
```
