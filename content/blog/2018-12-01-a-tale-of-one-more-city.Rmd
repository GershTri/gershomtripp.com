...
yelp_df %>% 
    ...
    ggtitle("Thirty Most-Reviewed Categories")
```
...
props_berlin_df <- yelp_df %>% 
    ...
    select(categories.title, Proportion) %>% 
    rename(Category = categories.title)

# function returns businesses with greater prevalence in district than in berlin
get_unique_businesses <- function(district_name, main_props_df = props_berlin_df){
    props_district <- yelp_df %>% 
        ...
        arrange(desc(n)) %>% 
        mutate(Proportion = round(n/sum(.$n), 3)) %>% 
        select(district, categories.title, Proportion) %>% 
        rename(Category = categories.title)

    props_diff_df <- left_join(props_district, main_props_df, by = "Category") %>% 
        mutate(Proportion.x = Proportion.x * 100,
               Proportion.y = Proportion.y * 100,
               diff = Proportion.x - Proportion.y) %>% 
        filter(diff > 0) %>% 
        arrange(desc(diff))
    
    fac <- factor(props_diff_df$Category[1:12],
                  ordered = T,
                  levels = props_diff_df$Category[1:12]
                  )
    fac
}
...
fried_kreuz_ord <- get_unique_businesses(district_name)
