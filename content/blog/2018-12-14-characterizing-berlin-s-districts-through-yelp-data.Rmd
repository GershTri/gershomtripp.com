...
What follows is an analysis of Yelp reviewed businesses in Berlin, Germany. It
can broadly be broken up into two parts: The first part constitutes a general
EDA where I explore the contours of the data and produce some basic statistics. 
The second part represents an attempt to describe each of Berlin's 12 
districts through "characteristic" businesses.
...
## A Brief EDA of Yelp-Reviewed Businesses in Berlin

***

This first section consitutes an exploratory analysis of Berlin's Yelp reviews,
and is meant to give us a feel for the data. 
...
A dot density plot can give some perspective on the overall distribution of Yelp-reviewed
businesses at the district and city level. The colors correspond to Berlin's 12 
administrative districts and Potsdam, a town to the southwest of Berlin. Knowing
exactly which color corresponds to which district isn't important at this point.
It's enough to realize that most business are concentrated in and around the 
city center, and that some districts obviously have more, and more geographically 
...
While the dot density plot looks cool and gives us some insight into the distribution
of all businesses in Berlin, it's not very efficient at showing the differences
between districts. For that I'll resort to the humble bar plot.

The following plot shows the absolute business count for each Berlin district. 
The districts with the highest number of businesses - Mitte, Panow, Friedrichshain-Kreuzberg, 
etc. - correspond to the densest clusters we saw in the previous dot density
chart, and those with the lowest number correspond to the outer, more residential 
districts


```{r plot_businesses_per_district, message=FALSE, warning=FALSE}
yelp_df <- yelp_df %>% 
    filter(district != "Potsdam")
...
yelp_df %>%
    ...
    ungroup() %>% 
    arrange(n) %>% 
    mutate(district = factor(district, ordered = T, levels = .$district)) %>% 
    ggplot(aes(x = district, y = n)) +
    geom_bar(stat = "identity", fill = "#7B241C" ) + 
    coord_flip() + 
    theme(panel.grid.major.y = element_blank()) + 
    scale_y_continuous(breaks = seq(0, 3500, by = 500), limits = c(0, 3500)) + 
    labs(y = "number of businesses",
         x = "district",
         title = "Total business count per district"
         )
```
