...
on Katie Hempenius's "incidence rate" metric in her 
[analysis](https://katiehempenius.com/post/yelp-map-of-san-francisco/) of San
Francisco's neighborhoods. I recommend reading Katie's post for a better understanding
of how she arrived at her metric, but we might summarize her reasoning thusly: 
Assuming the fraction of category~z~

With the dual goals of "identify[ing] interesting trends" and "normaliz[ing] for the effects of commercial versus residential 
...
Some will probably think that my first complaint is just hairsplitting, but think
it should be useful, nonetheless. Simply put, the above formula does not describe
the _incidence rate_. The [incidence rate](https://www.cdc.gov/ophss/csels/dsepd/ss1978/lesson3/section2.html) is a
measure of _new cases_ in a specified period of _time_. Time, however, doesn't
play a role in the above formula - nor should it, as we are really just interested
in a cross-section of Yelp-reviewed Berlin businesses at this point.

This doesn't at all invalidate Katie's, Ilia's, and Amber's work. "Incidence" is
commonly used to indicate the _occurence_ of something. In fact,
...
meaning than _incidence_ alone, and this is particularly relevant in the present
context, where an epidemiological approach is appropriate.

A less pressing issue, but perhaps relevant in terms of computational resources,
is the second half of the equation: It is a neighborhood-level constant, and has,
as such, no impact on the neighborhood-level rankings, which is what we are
interested in. A simplified formula will produce the same results:

$$
X = \frac{Category\;Z\;in\;zip}{Category\;Z\;in\;city}
$$

What remains is something that could probably be more accurately described as a
riff on the _prevalence rate_, where our "cases" are the number of businesses from
a category found in a given district, and our population consists of all category
members. Although useful, there are a number of problems with this metric when
it is used alone:

1. _It ignores differences in business frequency_. If 10 of 20 city-wide members of 
...
2. _It is biased in favor of small sample sizes_. Different types of businesses 
   likely aren't evenly distributed across a city. In particular, a small number
   of niche businesses might more likely be clustered in one area. For example,
   ...
   scarcity of some of its businesses.
...
4. _It ignores other indicators of interest_. It wouldn't be farfetched to view
   ...
   in reality, the same number of businesses from category c~x~, but there are more
   Yelp listings for that category in one d~1~ than d~2~, then we could
   hypothesize that people are more interested in c~x~ in d~1~ than in d~2~. It 
   would then be reasonable to conclude that c~x~ characterizes d~1~ _more_ than
   it does d~2~. But what if the c~x~ listings in d~2~ have more reviews on average?
