...
berlin_data %>% 
    ...
    labs(title = "Histograms of Temperature, 1941-1949",
         x = "Temperature"
         )
```


The [Battle of Berlin](https://en.wikipedia.org/wiki/Battle_of_Berlin) was in the spring of 1945, so there's really no question as to why the data is missing. That one outlier likely won't really effect any overall trends, but it makes sense to drop all data from 1945, nonetheless.
...
berlin_data %>% 
    ...
    ggplot(aes(x = Year, y = Value)) +
        ...
        labs(title = "Number of Days with Temp. > 25°C, 1957-2017",
             y = "Number of Days"
             )
```