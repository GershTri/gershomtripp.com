...
The average number of hours of daily sunshine appears to have remained fairly stable at least since 1977. There is some fluctuation, but there is no clear trend in any direction. I take this to mean that these months have not become significantly more overcast (or, conversely, sunnier).

There is, however, a clear downward trend for the months September and June from 1957 to about 1985. These two months appear to have lost about two hours each. I have no idea how that translates to daily cloud cover, but it's clear that June and September were once sunnier than they are today.
...
berdat_cortest <- psych::corr.test(x = berdat_sub, method = "spearman")

corrplot::corrplot(berdat_cortest$r,
...
```{r drop in air pressure greater than -5}
berlin_data %>% 
    filter(Variable == "Avg. Air Pressure (hPa)") %>% 
    filter_group() %>% 
    mutate(Value = (lag(Value) - Value)) %>% 
    filter(!is.na(Value), Value < -5) %>%
    group_by(Year, Month) %>% 
    summarize(Value = n()) %>% 
    gg_geoms_x_scale() + 
