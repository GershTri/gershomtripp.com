...
Predictably, the proportion of summer days with a maximum temperature above 25°C has gone up since 1957, in some cases dramatically. In August, for instance, the number has nearly doubled, equating to about a week more of warm days. July has seen a similar increase, and even May and September have seen the number of +25°C days go up. June seems to be a bit of an outlier as the number +25° days has remained relatively stable, although there has been some fluctuation.
...
The average number of hours of daily sunshine appears to have remained fairly stable at least since 1977. There is some fluctuation, but there is no clear trend in any direction. I take this to mean that these months have not become significantly more overcast (or, conversely, sunnier), at least not over the long term.
...
The average amount of daily precipitation also doesn't show any general upward trend since 1957. At shorter timescales, e.g. from about 2000 to 2017, there is a clear upturn in the months of June and July and a downturn in May, August and September. Part of June and July's increase can be explained by 2017, which was a historically wet year. That being said, the uptick remains even when 2017 is removed from the dataset.
...
Notwithstanding the increase in the average amount of daily precipitation, the proportion of "wet" to "dry" days has remained relatively stable, hovering around 50% for every month since 1957. Taken together, an increase in average daily precipitation, stable proportion of wet to dry days, and fairly stable amount of daily sunshine, seem to imply that storms have become more intense without becoming more frequent.
...
My lay intuition tells me that air pressure could be a further indicator of storm intensity. As I understand it, stable high pressure zones usually indicate good weather while rapidly decreasing air pressure indicates an impending storm. Hourly data are necessary to properly analyze the association between air pressure and bad weather, something I decided against for the this analysis for time reasons. Still, I assume strong storms would leave some trace by driving down daily averages. 

To see if air pressure, measured in hectopascals (hPa), is a good indicator of weather conditions, I'll plot the correlations of some relevant variables in a correlogram. I should note that this probably isn't the best method to determine how pressure affects weather. It's more of a heuristic to help me determine if there is a question here worth answering. Finer-grained air pressure data and variables more closely associated with bad weather, cloud type for instance, in addition to more rigorous statistical methods would allow me to more confidently draw conclusions.
...
Of particular interest are correlations between `PRESS` and the other variables that also make intuitive sense, as that would imply that air pressure is a good predictor of weather conditions. We should expect, for example, that high pressure is positively correlated with fair weather, as given by `SUN`, and negatively correlated with wind and bad weather, as given by `WIND_MAX`, `WIND_AVG`, and `PRECIP`. Looking at the plot, this seems to be the case, so I can accept that air pressure can tell me something about weather conditions - with the above proviso

The final plot displays the number of days per month where there is at least a -3 hPa drop in air pressure compared to the previous day. Although this measure is obviously not perfect, my layperson intuition tells me that an increase in the number of such drops might hint at an increase in storm fronts of a certain intensity. Feel free to let me know in the comments if this is a terribly misled idea.
...
```{r drop in air pressure greater than -3}
berlin_data %>% 
    ...
    scale_y_continuous(limits = c(2, 13),
                       breaks = seq(2, 13, by = 1)
                       ) +
    thm + 
    labs(title = "Drop in Air Pressure (hPa) from Previous Day > 5",
         y= "Number of Days"
         )    
```
